...
(function () {
    ...
    async function cargarRango(desde,hasta) {
        ...
        try {
            var r = await fetch('/api/roomservice/calendario?from='+desde+'&to='+hasta);
            if (!r.ok) return {};
    ...
    function colorBadge(info){ if(info&&info.conRoomService) return RS_COLOR; return '#c9a84c'; }
    ...
    function renderGridMes(a,m,conteos,lg) {
        ...
        for (var d=1;d<=dias;d++) {
            ...
            if (info&&info.conRoomService) cls += ' cal-celda--rs';
    ...
    function renderSemanal(content,conteos) {
        ...
        Promise.all(fechas.map(function(fecha){
            return fetch('/api/roomservice/calendario/dia?fecha='+fecha)
            ...
            resultados.forEach(function(item){
                ...
                var conRs = conteos[item.fecha]&&conteos[item.fecha].conRoomService;
                ...
                if (item.reservas.length===0) {
                    ...
                    reservasHtml = item.reservas.map(function(r){
                        var horaRs = (r.servicios||[]).filter(function(x){return x.esRoomService && x.hora;}).map(function(x){return x.hora.substring(0,5);}).join(',');
                        var pill = r.conRoomService
                            ? '<span style="display:inline-flex;align-items:center;gap:3px;vertical-align:middle;background:rgba(140,220,160,0.15);color:'+RS_COLOR+';border:1px solid rgba(140,220,160,0.3);border-radius:4px;padding:0px 6px;font-size:0.68rem;font-weight:600;">'+RS_SVG_SM+'RS'+(horaRs?' '+horaRs:'')+'</span>'
                            : '';
                ...
                html += '<div class="cal-sem-dia" data-day="'+item.fecha+'"'+(conRs?' style="border-left:3px solid rgba(140,220,160,0.5);"':'')+'>'
            ...
            content.querySelectorAll('.cal-sem-dia').forEach(function(el){
                el.addEventListener('click', function(){ rsDiaDetalle(el.dataset.day); });
            });
        });
    }
    ...
    // Bandeja con campana — Lucide bell-concierge
    var RS_SVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;">'
        + '<path d="M2 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2H2v2Z"/>'
        + '<path d="M20 16a8 8 0 1 0-16 0"/>'
        + '<path d="M12 4v4"/>'
        + '<path d="M10 4h4"/>'
        + '</svg>';
    var RS_SVG_SM = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;margin-right:2px;">'
        + '<path d="M2 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2H2v2Z"/>'
        + '<path d="M20 16a8 8 0 1 0-16 0"/>'
        + '</svg>';
    ...
    window.rsDiaDetalle = async function(fecha) {
        ...
        try {
            var r = await fetch('/api/roomservice/calendario/dia?fecha='+fecha);
