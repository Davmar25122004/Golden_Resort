...
(function () {
    ...
    async function cargarRango(desde, hasta) {
        ...
        try {
            var r = await fetch('/api/coche/calendario?from='+desde+'&to='+hasta);
            if (!r.ok) return {};
    ...
    function colorBadge(info){ if(info&&info.conCoche) return COCHE_COLOR; return '#c9a84c'; }
    ...
    function renderGridMes(a,m,conteos,lg) {
        ...
        for (var d=1;d<=dias;d++) {
            ...
            if (info&&info.conCoche) cls += ' cal-celda--coche';
    ...
    function attachGridClicks(root) {
        root.querySelectorAll('[data-day]').forEach(function(el){
            ...
            el.addEventListener('click', function(){
                ...
                cocheDiaDetalle(el.dataset.day);
            });
        });
    }
    ...
    function renderSemanal(content, conteos) {
        ...
        Promise.all(fechas.map(function(fecha){
            return fetch('/api/coche/calendario/dia?fecha='+fecha)
            ...
            resultados.forEach(function(item){
                ...
                var conCoche = conteos[item.fecha]&&conteos[item.fecha].conCoche;
                ...
                if (item.reservas.length===0) {
                    ...
                    reservasHtml = item.reservas.map(function(r){
                        var cocheSvc = (r.servicios||[]).find(function(x){return x.esCoche;});
                        var horaCoche = cocheSvc && cocheSvc.hora ? cocheSvc.hora.substring(0,5) : '';
                        var ubicCoche = cocheSvc && cocheSvc.ubicacion ? labelUbicacion(cocheSvc.ubicacion) : '';
                        var pill = r.conCoche
                            ? '<span style="display:inline-flex;align-items:center;gap:3px;vertical-align:middle;background:rgba(120,180,255,0.15);color:'+COCHE_COLOR+';border:1px solid rgba(120,180,255,0.3);border-radius:4px;padding:0px 6px;font-size:0.68rem;font-weight:600;">'+COCHE_SVG_SM+'COCHE'+(horaCoche?' '+horaCoche:'')+(ubicCoche?' · '+ubicCoche:'')+'</span>'
                            : '';
                        return '<div class="cal-sem-reserva">'
                            ...
                            + pillReserva(r)+pill+'</div>';
                    }).join('');
                }

                html += '<div class="cal-sem-dia" data-day="'+item.fecha+'"'+(conCoche?' style="border-left:3px solid rgba(120,180,255,0.5);"':'')+'>'
            ...
            content.querySelectorAll('.cal-sem-dia').forEach(function(el){
                el.addEventListener('click', function(){ cocheDiaDetalle(el.dataset.day); });
            });
        });
    }
    ...
    function labelUbicacion(u) {
        if (u === 'AEROPUERTO_VALENCIA')   return 'Aeropuerto VLC';
        if (u === 'RENFE_JOAQUIN_SOROLLA') return 'Renfe J. Sorolla';
        if (u === 'RENFE_CULLERA')         return 'Renfe Cullera';
        return u || '';
    }
    ...
    // Icono de coche (Lucide car)
    var COCHE_SVG = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;">'
        + '<path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/>'
        + '<circle cx="7" cy="17" r="2"/>'
        + '<path d="M9 17h6"/>'
        + '<circle cx="17" cy="17" r="2"/>'
        + '</svg>';
    var COCHE_SVG_SM = '<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;margin-right:2px;">'
        + '<path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/>'
    ...
    window.cocheDiaDetalle = async function(fecha) {
        ...
        try {
            var r = await fetch('/api/coche/calendario/dia?fecha='+fecha);
